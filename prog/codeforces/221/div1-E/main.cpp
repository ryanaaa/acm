...
int ent[maxn], ver[maxn << 1], nxt[maxn << 1], ENUM;
LL len[maxn << 1];

void addEdge(int u, int v, int w) {
	ver[ENUM] = v; len[ENUM] = w; nxt[ENUM] = ent[u]; ent[u] = ENUM++;
}

void downTop(int u, int par) {
	...
	if ((v = ver[i]) != par) {
		...
		for (int a = 1; a <= n; a++)
		if (u != a && dis[v][a] != -1)
			dis[u][a] = dis[u][v] + dis[v][a];
	}
}

void topDown(int u, int par, LL dis_upar) {
	if (par)
	for (int a = 1; a <= n; a++)
		if (u != a && dis[u][a] == -1)
			dis[u][a] = dis_upar + dis[par][a];
	for (int i = ent[u], v; ~i; i = nxt[i])
	if ((v = ver[i]) != par) {
		topDown(v, u, len[i]);
	}
...
int main() {
	while (~scanf("%d %d", &n, &x)) {
		...
		for (int u = 1; u <= n; u++) {
			scanf("%d", color + u);
			if (color[u] == 1) sumBlack++;
		}
		memset(ent, -1, sizeof(ent)); ENUM = 0;
		...
		// ∑(xij : dis[i,j]<=x) >= 1
		for (int u = 1; u <= n; u++) {
			for (int v = 1; v <= n; v++) if (dis[u][v] <= x)
				A[u - 1][v - 1] = -1;
			b[u - 1] = -1;
		}
		// ∑(xi) <= sumBlack
		for (int v = 1; v <= n; v++) A[n][v - 1] = 1;
		b[n] = sumBlack;

		for (int u = 1; u <= n; u++) {
			if (color[u] == 1) c[u - 1] = 1;
			else c[u - 1] = 0;
		}
		int flag = simplex.init(A, b, c, n, n + 1);
		if (flag) {
			puts("-1");	// nosolution
		} else {
			double sim = simplex.work();
			int res = (int) (sim + eps);
			printf("%d\n", sumBlack - res);
		}
	}
